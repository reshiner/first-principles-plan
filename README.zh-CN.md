...
1. **分解** — 理解用户的真实意图，批判性地评估现有代码或方案文档的设计债务和隐性假设
2. **设计** — 从第一性原理出发设计理想方案，不受现有代码或代码约束的影响
3. **调和** — 比较最小改动路径与理想设计，给出有理有据的推荐
4. **完善** — 通过迭代反馈协作修订分析，直到最终定稿

技能支持**两种模式**：**Standalone**（分析项目代码）和 **Review Gate**（审查 spec/plan/brainstorming 草案的假设和设计）。技能**并不总是推荐重构**。它的价值在于让权衡**显式化**——让你清楚地知道自己正在接受什么。

## 特性

- **两种调用模式** — **Standalone**（分析项目代码）和 **Review Gate**（审查 spec/plan/brainstorming 草案的假设和设计）
- **`/fpt` 斜杠命令**（Claude Code）或其它平台的等效触发方式
- **自动触发** — 在用户要求结构性质疑、说出"第一性原理"/"challenge assumptions"/"从根本分析"/"这个设计合理吗"，或传递源文档要求审查时自动激活
- **结构化输出** — 根据模式生成 `First Principles Analysis` 文档（Standalone）或 `FPT Review Feedback` 审查意见（Review Gate），包含意图分析、设计批判、理想方案、路径比较和推荐结论
...
### Review Gate 模式 — 审查方案/计划草案

传递源文档（brainstorming 产出、spec 或 plan），审查其假设和设计决策：
...
技能将生成结构化的 `FPT Review Feedback`，包含假设挑战、设计盲区和逐节修订建议。

### 保存分析文档到本地文件

Standalone 分析输出到对话后，工具会提示你保存为本地文件。只需说：
...
这个技能解决的是 AI 辅助开发中的特定问题：**LLM 天然倾向于最小改动路径**。训练数据（大多数 commit 是小改动）、系统提示（"外科手术式修改"）以及风险规避倾向共同导致了对修补而非修复的偏向——无论是代码还是设计文档。结果是质量通过累积不断衰减。
