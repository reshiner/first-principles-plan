...
- Claude Code: registered and loaded via the v2 plugin system (installed_plugins.json + settings.json)
...
The skill defines four required phases:
...
4. **Refine** (Iterative Refinement) — Post-analysis revision via conversation protocol: present the document, invite feedback, revise with versioning (v1.0 → v1.1 → ...), and exit on confirmation, save, or topic change
...
Analysis documents are saved to `docs/fpt/yyyymmdd-<topic>.md` on user request ("output document" / "保存"). The memory directory at `memory/` stores cross-session context (adapter architecture, output path conventions, iterative refinement behavior).
...
- **Refinement is conversation-driven** — no external state; the version counter and refinement mode are maintained via chat protocol
