...
| Trigger | Action |
|---------|--------|
| User says "确认" / "确认保存" / "接受" / "同意" / "finalize" / "confirm" / "accept" | **Confirm + Save + Exit:** Mark the current version as final, write to `docs/fpt/<date>-<summary>.md` using the Document Persistence rules, confirm success, and exit refinement mode. |
| User says "保存" / "导出" / "输出文档" / "save" / "export" | **Save + Exit:** Same as confirm — write to `docs/fpt/<date>-<summary>.md`, confirm success, and exit. (Saving implies finalization.) |
